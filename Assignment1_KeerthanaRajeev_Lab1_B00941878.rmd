...
The features which are used in the data dictionary are as follows:

1.PatientID : An identifier which can be used to uniquely identify a patient
2.AppointmentID : An identifier that uniquely identifies an appointment pertaining to a patient
3.Gender : The gender of the patient with values 'Female' and 'Male'
4.Scheduled Date : The date on which an appointment is scheduled
5.AppointmentDate : The date of the actual appointment
6.Age: The age of the patient
7.Neighbourhood: The place within the District of Victória where the appointment is scheduled
8.SocialWelfare: Depicts whether the Patient has received the Bolsa Família welfare payments
9.Hypertension: Indicates whether the patient was previously diagnoised with hypertension.It uses Boolean values
10.Diabetes: Represents whether the Patient was previously diagnosed with diabetes and uses Boolean value
11.AlcoholUseDisorder: Indicates whether the patient was previously diagnosed with alcohol use disorder. It has boolean values.
12.Disability: Indicates whether the patient was previously diagnosed with a disability. The severity is rated on a scale of 0-4.
13.SMSReceived: Represents whether the patient was sent least 1 reminder text before appointment. Uses Boolean values.
14.NoShow: Indicates whether the patient has attended the appointment or not and has boolean values Yes and No 

**2** Can you think of 3 hypotheses for why someone may be more likely to miss a medical appointment?
1. Accessibility : The lack of accessibility such as transportation facilities which can hinder a person from attending their medical appointment.

2.Low-income families: People who hail from poor families are dependent of the wages they make and therefore cannot afford to miss their job. If they have to work on the time slot of the medical appointment and this decision was informed by the workplace in the last minute, they will have to miss their appointments.

3.Emergencies : Last Minute emergencies that can occur might cause a person to miss their appointment.
Eg: Death of next to kin
...
1. The type of specialized doctor that they are meeting up with. For eg: Pediatrician, Orthopaedic surgeon, Cardiologist etc.

2. Insurance : Indicating whether the patient has insurance or not and with which company. Can have boolean values.

3. Drug Usage : A boolean variable with values Yes or no indicating whether the person has used drugs before or not.
...
raw.data <- readr::read_csv('https://raw.githubusercontent.com/keerthurajeev9/AppliedResearch/main/2016_05v2_VitoriaAppointmentData.csv',col_types='fffTTifllllflf')
```
...
**5** Are there any individuals with impossible ages? If so we can drop this row using
`filter` i.e., `data <- data %>% filter(CRITERIA)`
Uisng the below code, we can drop the rows with impossible values. Yes, there are people with impossible ages.
```{r}
...
From the heatmap, it is quite clear that SMSReceieved with a co-relation coefficient value of 0.13 strongly correlated with missing appointments 'NoShow'

**8** Are there any other variables which strongly correlate with one another?

The attributes PatientID and AppointmentID strongly co-relates with each other with a value of 0.65(r)

**9** Do you see any issues with PatientID/AppointmentID being included in this plot? 

Since both attributes are identifiers which are random and numerical values, they don't provide sufficient information although there's a strong correlation between them.
They are also categorical variables and not continuous in nature and correlation is more suited for the former.
...
If we didn't plot the first graph, we would think that the data is equally distributed across Age  while the first graph reveals that the data i'snt equally distributed.

Also in the first graph, the partition for 100-110 and 110-120 is empty while in the second plot the range 110-120 for Age Range has some data plotted with respect to NoShow.
...
According to the plot, SMS reminders reduce the likelihood that someone will forget to show up for an appointment(NoShow). 

The data may have been biassed in this way since those who are more likely to keep an appointment may also be more likely to opt-in to get SMS reminders since SMS can remind people to attend the appointment and act as an alert/notification.
...
ggplot(raw.data) + 
  ...
  ggtitle("Density of No show received across Disability")
ggplot(raw.data) + 
...
A major reason could be the accessibility issues. People hailing from different neighborhoods could be affected by the variance in the transportation facilities for example places with limited infrastructure might find it extremely difficult to reach the appointment centers on time.
Factors such as social and economic can also affect the attendance rate.Eg: Financial contraints
...
Since the model has been cleaned thoroughly, split into test and train split, feature engineering applied, the model should work well.
...
fit.control <- trainControl(number = 5,search = "grid", method = "cv", summaryFunction = twoClassSummary, classProbs = TRUE)

models.randomforest <- train(NoShow ~ ., data = train, method = "rf", metric = "ROC", trControl = fit.control)

models.pred <- predict(models.randomforest, newdata = test)
models.probs <- predict(models.randomforest, newdata = test, type = "prob")

confusionMatrix(models.pred, test$NoShow, positive = "Yes")
paste("Random Forest with performance metric ROC: ", round(auc(test$NoShow.numerical, models.probs[, 2]), 3), sep = "")

```

**18** Based on everything, do you think we can trust analyses based on this dataset? Explain your reasoning.

The below criteria can be used to rate the validity of analyses based on this dataset:

a.The data dictionary provides us information about all the attributes in the dataset and their meanings which is the initial step in understanding and interpreting the variable.

b.Parsing the data and cleaning it(Pre-processing) : Performing some initial processing to the data identifying the errors in the data eg: removing ages which are irrelevant to processing. Also ensuring that the code is reproducible by reading the data directly from git

c.Feature Engineering : Creating or transformation of new variables/existing from existing ones and identifying attributes that can improve the ML model and the output variable.

d. Correlation: A heatmap is constructed which shows the relationship between different variables in a visualization and uses r to identify patterns.
Eg: SMS received had a positive correlation with output variable 'Noshow' while PatientID and AppointmentID are strongly correlated.

e. Training and Evaluating the model: Train and tests split are generated from the data and XGBoost is used as a model to evaluate the performance.Performance metrics such as Area under curve and a confusion matrix is constructed. We have also used Random Forest as a model and then evaluated it using various performance metrics.

As per the above knowledge, we can trust this analysis to some extent. Exploratory data analysis has been performed however there are some attributes that can be added to the dataset.

```{r}
# Specify the filename for your R Markdown file
Assignment <- "Assignment1_KeerthanaRajeev_Lab1_B00941878.Rmd"

# Knit the R Markdown file
knit(Assignment)

# Convert the Markdown file to PDF
rmarkdown::render(paste0(tools::file_path_sans_ext(Assignment), ".md"))
