...
RegressionModel <- train(NoShow ~ .,data=train, method="glm", family = "binomial",trControl=ctrl)

print(RegressionModel)
logit.pred <- predict(RegressionModel, newdata=test)

logit.probs <- predict(RegressionModel, newdata=test, type="prob")
...
paste("Logistic Regression with performance metric ROC: ", round(auc(test$NoShow.numerical, logit.probs[,2]),3), sep="")
...
d.Correlation: A heatmap is constructed which shows the relationship between different variables in a visualization and uses 'r'(coefficient of correlation) to identify patterns.
